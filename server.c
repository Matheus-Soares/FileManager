...
int main(){
	...
	socketAddr.sin_addr.s_addr = inet_addr("127.0.0.1");	// Endereço IP (inet_addr converte o endereço para binario em bytes)
	bzero(&(socketAddr.sin_zero), 8);						// Estrutura com zeros sem utilidade (Para propósitos futuros)?

	int socketR = socket(AF_INET, SOCK_STREAM, 0);  	// Initializa o socket. 	(Domínio da comunicação (AF_NET p/ TCP/IP),
